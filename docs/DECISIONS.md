...
## 2026-09-15：PR #586–#588 判讀，commit 軸無新增

commit 水位維持 `5b2c000`（`--strict` 實查無新 commit）；PR 水位 583 → 588；issue 水位維持 569。

### 不引用：#586 Glasser 整合指南、#587 AIPM registry 徽章

兩筆都是第三方廠商在上游掛自己的導流入口：#586 新增 `tools/integrations/glasser.md` 與
`tools/REGISTRY.md` 一列（付費 API broker，227 行廠商說明），#587 在 `README.md` 加 AIPM
registry 徽章與第 7 種安裝選項。本 fork 對這兩家廠商同樣沒有商業關係，`README` 既有政策是
「作者宣傳、贊助 CTA、個人事業連結略過」，廠商整合卡片與第三方登錄徽章屬同一類。這與
2026-08-30 對 Converly、2026-09-11 對 Ploy 的處理一致。

**觸發條件**：政策性不引用，不是等待中的暫緩；除非維護者決定本線要開始轉載廠商整合內容。
